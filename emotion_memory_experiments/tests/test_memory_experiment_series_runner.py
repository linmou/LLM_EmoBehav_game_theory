...
spec = importlib.util.spec_from_file_location(
    "memory_experiment_series_runner", runner_path
)
...
class TestMemoryExperimentSeriesRunner(unittest.TestCase):
    ...
    def test_expand_benchmark_configs_with_literal_task_type(self):
        ...
        benchmarks = [
            {"name": "test_bench", "task_type": "literal_task", "sample_limit": 100}
        ]
    ...
    def test_expand_benchmark_configs_with_pattern_task_type_now_works(self):
        ...
        with patch.object(
            BenchmarkConfig, "discover_datasets_by_pattern"
        ) as mock_discover:
    ...
    def test_error_handling_pattern(self):
        ...
        for i, exp in enumerate(experiments):
            try:
                ...
                failed_experiments.append(exp["exp_id"])
                error_msg = (
                    f"🚨 SERIES-LEVEL ERROR for experiment {exp['exp_id']}: {str(e)}"
                )
                mock_logger_error(error_msg)

                try:
                    mock_update_experiment_report(
                        exp["exp_id"],
                        status="FAILED",
        ...
        self.assertIn(
            "exp_2", failed_experiments, "exp_2 should fail (experiment-level failure)"
        )
        self.assertIn(
            "exp_3", failed_experiments, "exp_3 should fail (series-level error)"
        )
    ...
    def test_benchmark_expansion_error_handling(self):
        ...
        original_benchmarks = [
            {
                "name": "bench1",
                "task_type": ".*qa.*",
            },  # Pattern that might cause issues
        ...
        self.assertEqual(
            benchmarks,
            original_benchmarks,
            "Should fallback to original benchmarks on expansion error",
        )
        self.assertEqual(
            len(errors_logged), 2, "Should log expansion error and fallback message"
        )
    ...
    def test_experiment_generation_error_handling(self):
        ...
        self.assertEqual(
            len(generated_experiments), 2, "Should generate 2 valid experiments"
        )
        self.assertEqual(
            len(errors_logged),
            2,
            "Should log 2 generation errors for problematic model",
        )
    ...
    def test_comprehensive_error_handling_behavior(self):
        ...
        test_experiments = [
            {
                "exp_id": "benchmark1_task1_model1",
                "benchmark_name": "benchmark1_task1",
                "model_name": "model1",
            },
            {
                "exp_id": "benchmark1_task1_model2",
                "benchmark_name": "benchmark1_task1",
                "model_name": "model2",
            },
            {
                "exp_id": "benchmark2_task2_model1",
                "benchmark_name": "benchmark2_task2",
                "model_name": "model1",
            },
            {
                "exp_id": "benchmark2_task2_model2",
                "benchmark_name": "benchmark2_task2",
                "model_name": "model2",
            },
        ]
        ...
        for i, exp in enumerate(test_experiments):
            try:
                ...
                if not mock_check_model_existence(exp["model_name"]):
                    mock_update_experiment_report(
                        exp["exp_id"], status="FAILED", error="Model not found"
                    )
                    continue

                # Run individual experiment
                success = mock_run_single_experiment(
                    None, exp["model_name"], exp["exp_id"]
                )
        ...
        self.assertEqual(
            len(attempted_experiments), 4, "All 4 experiments should be attempted"
        )
        self.assertEqual(
            len(successful_experiments), 2, "Should have 2 successful experiments"
        )
        self.assertEqual(
            len(failed_experiments),
            2,
            "Should have 2 failed experiments (1 individual + 1 series error)",
        )
        self.assertEqual(len(series_errors), 1, "Should log 1 series-level error")

        # Verify specific experiments failed as expected
        self.assertIn(
            "benchmark1_task1_model1",
            failed_experiments,
            "First experiment should fail (individual failure)",
        )
        self.assertIn(
            "benchmark2_task2_model1",
            failed_experiments,
            "Third experiment should fail (series error)",
        )

        # Verify successful experiments
        self.assertIn(
            "benchmark1_task1_model2",
            successful_experiments,
            "Second experiment should succeed",
        )
        self.assertIn(
            "benchmark2_task2_model2",
            successful_experiments,
            "Fourth experiment should succeed",
        )
    ...
    def test_format_model_name_for_folder(self):
        ...
        test_cases = [
            ...
            ("org/model", "org/model"),
            (
                "/data/home/huggingface_models/RWKV/v6-Finch-7B-HF",
                "RWKV/v6-Finch-7B-HF",
            ),
    ...
    def test_pipeline_worker_error_handling_pattern(self):
        ...
        self.assertEqual(
            len(processed_batches), 3, "All 3 batches should be processed despite error"
        )
        ...
        self.assertEqual(
            len(pipeline_queue), 4, "Should have 3 batch results + 1 sentinel"
        )
        ...
        self.assertIn(
            None,
            pipeline_queue,
            "Sentinel value should be present to prevent main thread hanging",
        )
    ...
    def test_main_thread_worker_error_handling(self):
        ...
        self.assertEqual(
            len(processed_items), 1, "Should process 1 normal batch before worker error"
        )
    ...
    def test_experiment_series_continues_after_failure(
        self, mock_run_single, mock_check_model
    ):
        ...
        self.assertEqual(
            summary["total"], expected_total, "Report should track all experiments"
        )
        self.assertEqual(summary["failed"], 1, "Report should show 1 failed experiment")
        self.assertEqual(
            summary["completed"],
            expected_total - 1,
            "Report should show 3 completed experiments",
        )
