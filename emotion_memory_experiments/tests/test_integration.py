...
from .test_utils import (
    create_mock_passkey_data, create_temp_data_file, 
    MockRepControlPipeline, cleanup_temp_files
)
...
class TestIntegration(unittest.TestCase):
    ...
    def test_full_passkey_experiment_workflow(self, mock_get_pipeline, mock_load_emotion_readers,
        ...
        mock_anger_reader.directions = {layer: np.array([0.1, 0.2]) for layer in range(-1, -9, -1)}
        mock_anger_reader.direction_signs = {layer: np.array([1, -1]) for layer in range(-1, -9, -1)}
        
        mock_happiness_reader = MagicMock()
        mock_happiness_reader.directions = {layer: np.array([0.2, 0.3]) for layer in range(-1, -9, -1)}
        mock_happiness_reader.direction_signs = {layer: np.array([-1, 1]) for layer in range(-1, -9, -1)}
        ...
        required_columns = ['emotion', 'intensity', 'item_id', 'task_name', 'score', 'benchmark']
    ...
    @patch('emotion_memory_experiments.experiment.load_emotion_readers')
    @patch('emotion_memory_experiments.experiment.get_pipeline')
    def test_sanity_check_workflow(self, mock_get_pipeline, mock_load_emotion_readers,
                                  mock_model_detector, mock_setup_model):
        ...
        mock_emotion_reader.directions = {layer: np.array([0.1]) for layer in range(-1, -7, -1)}
        mock_emotion_reader.direction_signs = {layer: np.array([1]) for layer in range(-1, -7, -1)}
    ...
    @patch('emotion_memory_experiments.experiment.load_emotion_readers')
    @patch('emotion_memory_experiments.experiment.get_pipeline')
    def test_different_batch_sizes(self, mock_get_pipeline, mock_load_emotion_readers,
                                  mock_model_detector, mock_setup_model):
        ...
        mock_emotion_reader.directions = {layer: np.array([0.1]) for layer in range(-1, -5, -1)}
        mock_emotion_reader.direction_signs = {layer: np.array([1]) for layer in range(-1, -5, -1)}
        mock_load_emotion_readers.return_value = {"anger": mock_emotion_reader}
