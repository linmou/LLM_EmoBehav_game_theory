...
class TestPipelineWorkerKeyError(unittest.TestCase):
    ...
    def test_pipeline_worker_keyerror_with_real_collate_fn(self, mock_get_pipeline, mock_load_emotion_readers,
        ...
        real_pipeline.side_effect = lambda prompts, **kwargs: [
            MagicMock(outputs=[MagicMock(text="test response")])
            for _ in prompts
        ]
    ...
    @patch('emotion_memory_experiments.experiment.load_emotion_readers')
    @patch('emotion_memory_experiments.experiment.get_pipeline')
    def test_post_process_batch_keyerror(self, mock_get_pipeline, mock_load_emotion_readers,
                                        mock_model_detector, mock_setup_model):
        """
        ...
        with self.assertRaises(KeyError) as context:
            experiment._post_process_memory_batch(batch, control_outputs, 0)
