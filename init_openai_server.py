...
class ChatCompletionRequest(BaseModel):
    ...
    stop: Optional[List[str]] = Field(default=None, description="Up to 4 sequences where generation will stop")
...
def load_emotion_activation_vectors(model_path: str, emotion: str) -> Dict[int, torch.Tensor]:
    ...
    if emotion not in emotion_readers:
        raise ValueError(f"Emotion '{emotion}' not found in loaded readers. Available: {list(emotion_readers.keys())}")
...
def initialize_model(model_path: str, emotion: str, tensor_parallel_size: int = 1):
    ...
    activations, control_layers, emotion_readers = load_emotion_activation_vectors(model_path, emotion)
...
async def stream_chat_completion(completion_id: str, created_time: int, model: str, full_text: str, prompt_tokens: int, completion_tokens: int):
    ...
    try:
        ...
        if request.stream:
            return StreamingResponse(
                stream_chat_completion(completion_id, created_time, request.model, generated_text, prompt_tokens, completion_tokens),
