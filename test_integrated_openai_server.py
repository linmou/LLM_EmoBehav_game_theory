...
class IntegratedServerTester:
    ...
    def test_emotion_influenced_responses(self):
        ...
        if all(len(r) > 0 for r in responses):
            logger.info(f"✅ All {len(responses)} emotion-influenced responses generated successfully")
            return True
        else:
    ...
    def test_concurrent_requests(self):
        ...
        def make_request(prompt_id):
            try:
                completion = self.client.chat.completions.create(
                    model=self.model_name,
                    messages=[{"role": "user", "content": f"This is request {prompt_id}. Respond briefly."}],
                    max_tokens=20,
                    temperature=0.0
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Integrated test for OpenAI-compatible RepControlVLLMHook server")
    parser.add_argument("--model", 
                       default="/data/home/jjl7137/huggingface_models/Qwen/Qwen2.5-0.5B-Instruct",
                       help="Path to the model")
    parser.add_argument("--model_name", 
                       default="qwen3-0.5B-anger",
                       help="Model name for API")
    parser.add_argument("--emotion", 
                       default="anger",
                       help="Emotion to test")
    ...
    try:
        ...
        if success:
            ...
            logger.info(f"client = OpenAI(base_url='http://localhost:{args.port}/v1', api_key='token-abc123')")
            logger.info(f"response = client.chat.completions.create(model='{args.model_name}', messages=[{{'role': 'user', 'content': 'Hello!'}}])")
