"""
result_analysis/trust_game_expected_score.py

Shared implementation for:
- Trust Game (Trustor) expected-score delta vs neutral
- Trust Game (Trustee) expected-score delta vs neutral
"""
...
TRUSTOR_SPEC = TrustGameSpec(
    benchmark_substring="Trust_Game_Trustor",
    output_prefix="trustor",
    title_role="Trustor",
    decision_encoding_md="Decision encoding used: `trust_none=0`, `trust_low=1`, `trust_high=2`.",
    up_label="more trust",
    down_label="less trust",
    behavior_to_score={"trust_none": 0, "trust_low": 1, "trust_high": 2},
)

TRUSTEE_SPEC = TrustGameSpec(
    benchmark_substring="Trust_Game_Trustee",
    output_prefix="trustee",
    title_role="Trustee",
    decision_encoding_md="Decision encoding used: `return_none=0`, `return_medium=1`, `return_high=2`.",
    up_label="more return",
    down_label="less return",
    behavior_to_score={"return_none": 0, "return_medium": 1, "return_high": 2},
)
...
def _iter_run_dirs(report: dict[str, Any], spec: TrustGameSpec) -> Iterable[tuple[str, Path]]:
...
def _load_records(run_dir: Path, model: str, spec: TrustGameSpec) -> list[dict[str, Any]]:
...
def _compute_item_expected_score_deltas(df: pd.DataFrame, spec: TrustGameSpec) -> pd.DataFrame:
...
def _compute_item_max_delta_summary(rate: pd.DataFrame, spec: TrustGameSpec) -> pd.DataFrame:
...
def _write_report_md(
    ...
    lines: list[str] = []
    lines.append(f"# Trust Game ({spec.title_role}) — Expected decision score delta vs neutral")
    lines.append("")
    ...
    lines.append("")
    cols = ["model", "item_id", "neutral_decision_score", "max_inc_emotion", "max_inc_intensity", "max_inc_score", "max_inc_delta"]
    lines.append(md_table(inc[cols], max_rows=15))
    lines.append("")
    lines.append(f"### Top decreases (toward {spec.down_label})")
    lines.append("")
    cols = ["model", "item_id", "neutral_decision_score", "max_dec_emotion", "max_dec_intensity", "max_dec_score", "max_dec_delta"]
...
def run_from_report(report_path: Path, out_dir: Path | None, spec: TrustGameSpec) -> AnalysisOutputs:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--role",
        choices=["trustor", "trustee"],
        default=None,
        help="Limit analysis to one role (default: run both trustor and trustee)",
    )
    ...
    spec = TRUSTOR_SPEC if args.role == "trustor" else TRUSTEE_SPEC
    run_from_report(report_path=args.report, out_dir=args.out_dir, spec=spec)
    return 0
