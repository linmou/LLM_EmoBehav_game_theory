"""
result_analysis/trust_game_trustee_expected_score.py

Compute item-level decision shift vs neutral for Trust Game (Trustee).
...
Output (written into an output directory):
  ...
  - trustee_item_expected_score_max_delta_summary.csv
...
_BEHAVIOR_TO_SCORE = {"return_none": 0, "return_medium": 1, "return_high": 2}
...
def _iter_trustee_run_dirs(report: dict[str, Any]) -> Iterable[tuple[str, Path]]:
    ...
    for exp in experiments.values():
        ...
        if not isinstance(benchmark_name, str) or "Trust_Game_Trustee" not in benchmark_name:
            continue
...
def _load_trustee_records(run_dir: Path, model: str) -> list[dict[str, Any]]:
...
def run_from_report(report_path: Path, out_dir: Path | None = None) -> AnalysisOutputs:
    ...
    rows: list[dict[str, Any]] = []
    for model, run_dir in _iter_trustee_run_dirs(report):
        rows.extend(_load_trustee_records(run_dir=run_dir, model=model))

    if not rows:
        raise ValueError(f"No Trust_Game_Trustee runs found in {report_path}")
    ...
    out_rate = out_dir / "trustee_item_expected_score_delta_vs_neutral.csv"
    out_item = out_dir / "trustee_item_expected_score_max_delta_summary.csv"
    out_agg = out_dir / "trustee_expected_score_delta_aggregate_by_emotion_intensity.csv"
    out_agg_e = out_dir / "trustee_expected_score_delta_aggregate_by_emotion.csv"
    out_md = out_dir / "trustee_item_expected_score_delta_report.md"
...
def _compute_item_expected_score_deltas(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for beh in ["return_none", "return_medium", "return_high"]:
        tmp = (
    ...
    neutral = (
        ...
        .set_index(["model", "item_id"])[["decision_score_mean", "p_return_none", "p_return_medium", "p_return_high"]]
        .rename(
            columns={
                "decision_score_mean": "neutral_decision_score",
                "p_return_none": "neutral_p_return_none",
                "p_return_medium": "neutral_p_return_medium",
                "p_return_high": "neutral_p_return_high",
            }
        )
    )
    ...
    # Some runs may have missing/invalid neutral rows for a small subset of items; skip those items.
    ...
    neutral = (
        rate.drop_duplicates(subset=idx_cols)[
            ...
            + [
                ...
                "neutral_p_return_none",
                "neutral_p_return_medium",
                "neutral_p_return_high",
            ]
        ]
        .set_index(idx_cols)
    )
...
def _write_report_md(
    ...
    lines: list[str] = []
    lines.append("# Trust Game (Trustee) — Expected decision score delta vs neutral")
    lines.append("")
    lines.append("Decision encoding used: `return_none=0`, `return_medium=1`, `return_high=2`.")
    lines.append("")
    lines.append("## Output files")
    lines.append("")
    lines.append("- `trustee_item_expected_score_delta_vs_neutral.csv`")
    lines.append("- `trustee_item_expected_score_max_delta_summary.csv`")
    lines.append("- `trustee_expected_score_delta_aggregate_by_emotion_intensity.csv`")
    lines.append("- `trustee_expected_score_delta_aggregate_by_emotion.csv`")
    lines.append("")
    ...
    lines.append("")
    lines.append("### Top increases (toward more return)")
    lines.append("")
    cols = [
        "model",
        "item_id",
    ...
    lines.append("")
    lines.append("### Top decreases (toward less return)")
    lines.append("")
    cols = [
        "model",
        "item_id",
