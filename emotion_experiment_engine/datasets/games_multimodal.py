"""Multimodal dataset adapter for game theory benchmarks (image + text).

This is intentionally minimal: it reuses `GameTheoryDataset` parsing/evaluation,
and only adds the ability to carry image paths and load PIL images in `__getitem__`.
"""
...
from .games import GameTheoryDataset, REPO_ROOT, get_game_config
...
class GameTheoryMultimodalDataset(GameTheoryDataset):
    ...
    def _load_and_parse_data(self) -> List[BenchmarkItem]:
        ...
        for item in items:
            ...
            md["image_paths"] = paths
            md["image_path"] = paths[0]
            item.metadata = md
