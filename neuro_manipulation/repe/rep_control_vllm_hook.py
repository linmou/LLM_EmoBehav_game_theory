...
def hook_fn_rep_control(module, args, output):
    ...
    if full_controller is None or operator_name is None:
        logger.warning(f"Rank {rank} - Module {module.__class__.__name__} - RepControl state incomplete (missing controller or operator_name), skipping.")
        return output
    ...
    try:
        ...
        elif mask is None and "position_ids" in state.get('kwargs', {}): # Check if kwargs were passed
        ...
        elif tp_size > 1 and modified_dim == full_dim // tp_size: # TODO: Test if this branch is correct
        ...
        if len(controller_slice.shape) == 1:
            ...
            if controller_slice.shape[1] == 1:
                ...
                logger.warning(f"Rank {rank} - Controller sequence length ({controller_slice.shape[1]}) does not match activation sequence length ({modified.shape[0]}) and is not 1. Trying broadcast.")
        ...
        if isinstance(token_pos, int):
            modified[:, token_pos] = operator_fn(modified[:, token_pos], controller_masked[:, token_pos])
            ...
            if token_pos == "end":
                modified[:, -len_token:] = operator_fn(modified[:, -len_token:], controller_masked[:, -len_token:])
        ...
        logger.error(f"Rank {rank} - Error in RepControl hook for {module.__class__.__name__}: {e}", exc_info=True)
...
def _find_target_module(worker_self, layer_index, block_name):
    ...
    layers = ModelLayerDetector.get_model_layers(model) # Expects list of top-level layers
    if not layers or layer_index >= len(layers):
         logger.warning(f"RPC: Worker Rank {worker_self.rank} - Layer index {layer_index} out of bounds ({len(layers)} layers found).")
         return None
    ...
    if block_name == "decoder_block": # Assuming 'decoder_block' means the main layer module
         return target_layer
    elif hasattr(target_layer, block_name):
         return getattr(target_layer, block_name)
    else:
         logger.warning(f"RPC: Worker Rank {worker_self.rank} - Could not find block '{block_name}' within layer {layer_index} ({target_layer.__class__.__name__}). Targeting layer itself.")
...
def _register_hook_on_worker_rpc(worker_self, layer_index, block_name, hook_func):
    ...
    try:
        ...
        logger.error(f"RPC: Worker Rank {rank} error during hook registration: {e}", exc_info=True)
        return False
...
def _set_controller_state_on_worker_rpc(worker_self, layer_index, block_name, state):
    ...
    try:
        ...
        logger.error(f"RPC: Worker Rank {rank} error setting controller state: {e}", exc_info=True)
        return False
...
def _reset_controller_state_on_worker_rpc(worker_self, layer_index, block_name):
    ...
    try:
        ...
        logger.error(f"RPC: Worker Rank {rank} error resetting controller state: {e}", exc_info=True)
        return False

# --- Main Class ---
class RepControlVLLMHook:
    def __init__(self, model: LLM, tokenizer, layers: list[int], block_name: str, control_method: str, tensor_parallel_size: int = 1):
        """
        ...
        self.hook_handles = {} # Store handles locally if needed, but primary management is RPC
        ...
        if hasattr(self.model.llm_engine, 'parallel_config') and hasattr(self.model.llm_engine.parallel_config, 'tensor_parallel_size'):
            ...
            logger.warning("Could not detect tensor_parallel_size from engine's parallel_config during init. Assuming tp_size=1.")
        # --- Get and store Tensor Parallel Size --- End

        logger.info(f"Initializing RepControlVLLMHook for layers {layers}, block '{block_name}' with TP size: {self.tp_size}.")

        # Register the hook on all target layers via RPC
        for layer_id in self.layers:
            logger.info(f"Registering hook for layer {layer_id}, block '{block_name}'...")
            rpc_results = self.model.llm_engine.collective_rpc(
                _register_hook_on_worker_rpc,
                args=(layer_id, self.block_name, hook_fn_rep_control) # Pass hook fn directly
            )
            logger.info(f"RPC hook registration results for layer {layer_id}: {rpc_results}")
            if not any(rpc_results):
                logger.warning(f"Failed to register hook on any worker for layer {layer_id}. Control might not work.")
    ...
    def _get_operator_fn(self, operator_name='linear_comb'):
        ...
        if operator_name == 'linear_comb':
            ...
            # Note: This requires controller to be compatible shape for sum
            return lambda current, controller: current + controller * torch.sign((current * controller).sum(-1, keepdim=True))
    ...
    def __call__(self,
        ...
        try:
            ...
            if control_active:
                logger.info(f"Setting controller state for layers {list(activations.keys())}...")
                set_results = []
                for layer_id, activation_tensor in activations.items():
                    if layer_id not in self.layers:
                        logger.warning(f"Layer {layer_id} in activations not in initialized layers {self.layers}. Skipping.")
                        continue
                    ...
                    logger.debug(f"Sending state for layer {layer_id} with tp_size: {state['tp_size']}")
                    ...
                    set_results.append(all(rpc_results)) # Track if successful on all workers
                    logger.debug(f"RPC set state results for layer {layer_id}: {rpc_results}")

                if not all(set_results):
                    logger.warning("Failed to set controller state on some workers/layers. Control might be inconsistent.")
            ...
            # 3. Run generation
            logger.info(f"Running generation with{' control active' if control_active else 'out control'}...")
            ...
            if control_active:
                logger.info(f"Resetting controller state for layers {list(activations.keys())}...")
                reset_results = []
                # Reset state only for layers where it was potentially set
                for layer_id in activations.keys():
                     if layer_id not in self.layers: continue # Skip layers not managed by this instance
                     ...
                     logger.debug(f"RPC reset state results for layer {layer_id}: {rpc_results}")
                if not all(reset_results):
                     logger.warning("Failed to reset controller state on some workers/layers.")
    ...
    def remove_hooks(self):
         ...
         logger.warning("Hook removal is experimental and may require specific handle management.")
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
         ...
         num_gpus = min(num_gpus, detected_gpus) # Use specified or max available, whichever is smaller
    else:
    ...
    dummy_vec_dim = 4096 # Adjust to the model's hidden size (e.g., Llama3.1-8B is 4096)
    ...
    try:
        ...
        llm = LLM(model=model_name,
                  ...
                  max_num_seqs=16) # Adjust as needed
        logger.info("LLM loaded successfully.")

        # --- Initialize RepControl ---
        logger.info(f"Initializing RepControlVLLMHook for layers {layers_to_control}, block '{block_to_hook}'...")
        rep_control = RepControlVLLMHook(llm, tokenizer, layers_to_control, block_to_hook, control_method)
        logger.info("RepControlVLLMHook initialized.")

        # --- Baseline Generation ---
        logger.info("--- Running Baseline Generation ---")
        baseline_outputs = rep_control([prompt], max_new_tokens=10, temperature=0.0) # No activations passed
        ...
        for layer_id in layers_to_control:
             ...
             control_activations[layer_id] = torch.ones(dummy_vec_dim, dtype=torch.float16) * 0.1 # Smaller magnitude
        ...
        if baseline_text != controlled_text:
             ...
             logger.warning("WARNING: Baseline and controlled outputs are the same. Control might not have worked as expected.")
