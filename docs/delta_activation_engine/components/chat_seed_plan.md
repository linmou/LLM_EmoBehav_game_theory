...
- `build_seeded_chat_jobs(base_cfg, model_paths, seeds, output_root)`: for each model path and seed, copies `loading_config`, injects `model_path` and `seed`, deep-copies `repe_eng_config`, clones `prompt_config`, and produces a new `DeltaActivationChatJobConfig` with shared emotions/intensities/output root.

## Key Logic
- Ensures immutability by copying nested dicts/lists; avoids side effects on the base config.
- Seeds are cast to `int` to prevent YAML/JSON string surprises.
