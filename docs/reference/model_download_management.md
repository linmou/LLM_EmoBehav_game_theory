...
**Path Structure Note:**
- When storing models in the alternative location (`../huggingface_models`), the directory structure mirrors the HuggingFace organization/model format. For example, the model `Qwen/Qwen2.5-7B-Instruct` will be stored at `../huggingface_models/Qwen/Qwen2.5-7B-Instruct`.
- This ensures that models from different organizations or with similar names are kept separate and organized, just as they are on HuggingFace.
