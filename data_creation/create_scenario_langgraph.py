...
async def main():
    with open("data_creation/persona_jobs.jsonl", "r") as f:
        persona_jobs = [json.loads(line)["item"] for line in f][30:]

    game_name = "Prisoners_Dilemma"
    scenario_path_base = f"data_creation/scenario_creation/langgraph_creation/scenarios/{game_name}_{datetime.now().strftime('%Y%m%d')}"
    ...
    for batch_index in tqdm(range(num_batches), desc="Processing Batches"):
        ...
        print("Saving scenarios for current batch...")
        ...
        print("Scenarios for current batch saved.")
