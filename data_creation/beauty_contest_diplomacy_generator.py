...
REQUIRED_CHOICE_KEYS = ("commit_0", "commit_1", "commit_2", "commit_3")
...
def build_prompt(
    ...
    return f"""
You are writing Diplomacy-themed scenarios that follow the Beauty Contest structure.
Avoid any wording that suggests a game or competition.
...
- game_category: choose one of ["BC2", "DogPile"]
...
Each behavior choice should be a concrete action or numeric commitment.
...
def build_seed(powers: list[str], now: Optional[float] = None) -> int:
    if now is None:
        now = time.time()
    normalized = [power.strip().lower() for power in powers if power and power.strip()]
    if not normalized:
        normalized = ["unknown"]
    normalized.sort()
    seed_input = f"{int(now)}|{','.join(normalized)}"
    digest = hashlib.sha256(seed_input.encode("ascii")).hexdigest()
    return int(digest[:12], 16)


def extract_powers_from_fewshot(fewshot: list[dict]) -> list[str]:
    powers: set[str] = set()
    for record in fewshot:
        ...
        for entry in participants:
            ...
            for chunk in tokens.split(","):
                if " and " in chunk.lower():
                    chunk = chunk.split(" and ", 1)[0]
                parts = chunk.strip().split()
                if not parts:
                    continue
                if "and" in parts:
                    parts = [p for p in parts if p.lower() != "and"]
                if parts and parts[-1].lower() in {"countries", "country", "allies", "allied"}:
                    parts = parts[:-1]
                if parts and parts[-1].lower() in {"other", "allied", "potential", "rivals"}:
                    parts = parts[:-1]
                if not parts:
                    continue
                power = " ".join(parts).strip()
                if power:
                    powers.add(power)
...
def validate_record(record: dict) -> dict:
    ...
    if record["game_name"] != "Beauty_Contest":
        raise ValueError("game_name must be Beauty_Contest")
    ...
    if not isinstance(participants, list) or len(participants) != 2:
        raise ValueError("participants must be a list with exactly two entries")
    for entry in participants:
    ...
    if set(behavior_choices.keys()) != set(REQUIRED_CHOICE_KEYS):
        raise ValueError("behavior_choices must contain commit_0..commit_3 only")
    for key in REQUIRED_CHOICE_KEYS:
        value = behavior_choices[key]
        if not isinstance(value, str) or not value.strip():
            raise ValueError(f"{key} must be a non-empty string")

    if not isinstance(record["game_category"], str) or not record["game_category"].strip():
        raise ValueError("game_category must be a non-empty string")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate diplomacy-themed Beauty Contest scenarios."
    )
    parser.add_argument(
        "--fewshot",
        type=Path,
        default=Path(
            "data_creation/scenario_creation/beauty_contest_fewshot_diplomacy.json"
        ),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(
            "data_creation/scenario_creation/langgraph_creation/Diplomacy_Beauty_Contest_all_data_samples.json"
        ),
    )
